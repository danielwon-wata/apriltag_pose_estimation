...
public:
	VisionPosPublisher() {
		...
		publisher->bind("tcp://localhost:5090");
	}

	void PublishVisionPos(long x, long y, long t) {
		// 메시지 생성
		std::string message = std::to_string(x) + "," + std::to_string(y) + "," + std::to_string(t);
		// 메시지 전송
...
struct PoseEstimationOutput {
	...
	float selected_distance;					// 선택된 마커와 카메라와의 거리
...
std::pair<long, std::string> sendGlobalPoseToBackend(const PoseData& pose, 
	int resultForBackend, const BasicInfoModel& basicInfo, int Heading) {
	json j;
...
PoseEstimationOutput poseEstimationFunction(
	...
	for (int i = 0; i < numDetections; i++) {
		...
		if (distance < min_distance) {
			...
			output.selected_distance = distance;
		...
		if (m_info.zx_change == 0) {
			...
			double g_angle3 = 360 - (std::atan2(pos_x, -pos_z) * 180 / CV_PI );
			angle3 = std::fmod(g_angle3, 360.0);
			if (angle3 < 0) {
				angle3 += 360.0;
			}
			...
			double g_angle3 = 360 - (std::atan2(pos_x, -pos_z) * 180 / CV_PI + 90);
			angle3 = std::fmod(g_angle3, 360.0);
			if (angle3 < 0) {
				angle3 += 360.0;
			}
			...
			double g_angle3 = 360 - (std::atan2(pos_x, -pos_z) * 180 / CV_PI + 180);
			angle3 = std::fmod(g_angle3, 360.0);
			if (angle3 < 0) {
				angle3 += 360.0;
			}
...
//    => R_global_cam = R_marker * R_cam
		...
		if (firstFrame) {
			...
			//double smooth_final_distance = calcDistance(finalPose, smoothedPose);
			//if (smooth_final_distance <= 0.5) {
			//	smoothedPose = finalPose;
			//}
		...
		if (id == output.selected_marker_id) {
			...
			//output.final_poses.push_back(finalPose);
			...
			//output.final_poses.push_back(currentPose);
		}
...
cv::Mat create2DMap(const std::vector<PoseData>& markerPoses,
	...
	const cv::Point& mapOrigin, double arrowLength = 20.0, int Heading=0) {
	...
	for (const auto& pose : markerPoses) {
		...
		std::ostringstream oss;
		oss << "ID: " << pose.marker_id/* << ",  " << pose.x << ", " << pose.z << ")"*/;
		cv::putText(mapImage, oss.str(), cv::Point(pixelX /*- 50*/, pixelY + 8 /*- 8*/), cv::FONT_HERSHEY_SIMPLEX, 0.3, cv::Scalar(0, 0, 0), 1);
	}
	...
	if (drawCamera) {
		...
		double camYaw = Heading * CV_PI / 180 ;
		cv::circle(mapImage, cv::Point(camPx + 2 * std::cos(camYaw), camPy + 2 * std::sin(camYaw)), 5, cv::Scalar(255, 0, 255), -1);
...
int main(int argc, char* argv[])
{
	...
	if (cameraMode == "mxid") {
		...
		VisionPosPublisher posPublisher;
		...
		while (true) {
			...
			std::cout << "[LOG] Front pose size: " << ", r_pose : " << outFront.raw_poses.size() << ", f_pose :" << outFront.final_poses.size() << "select distance: " << outFront.selected_distance << ", select id : " << outFront.selected_marker_id << std::endl;
			std::cout << "[LOG] Rear pose size: " << ", r_pose : " << outRear.raw_poses.size() << ", f_pose :" << outRear.final_poses.size() << "select distance: " << outRear.selected_distance << ", select id : " << outRear.selected_marker_id << std::endl;
			...
			bool noMarkersDetected = false;
			if (outFront.final_poses.size() == 0 && outRear.final_poses.size() == 0)
			{
				noMarkersDetected = true;
			}
			...
			if (outFront.final_poses.size() > 0)
			{
				F_D_T = true;
			}

			if (outRear.final_poses.size() > 0)
			{
				R_D_T = true;
			}
			...
			if (noMarkersDetected) {
				...
				mapImage = create2DMap(markerPoses, PoseData{}, false,
					/*drawCamera=*/false, mapSize, scale, mapOrigin, arrowLength, 0);

				std::pair<long, std::string> backendResponse = sendGlobalPoseToBackend(chosenCameraPose, resultForBackend, basicInfo, 0);
				...
				for (size_t i_f = 0; i_f < outFront.raw_poses.size(); i_f++) {
					...
					if (d_f < frontLocalMinDist) {
						...
						bestFrontIndex = i_f;
						std::cout << "id: " << outFront.selected_marker_id << std::endl;
				...
				*/
				float frontLocalMinDist = outFront.selected_distance;
				float rearLocalMinDist = outRear.selected_distance;
				...
				if (F_D_T && R_D_T) {
					...
					if (rearLocalMinDist < frontLocalMinDist) {		
						...
						chosenCameraPose = outRear.final_poses[0];
						...
						outRear.Heading = std::fmod(outRear.Heading + 180, 360);
						if (outRear.Heading < 0) {
							outRear.Heading += 360;
						}
						mapImage = create2DMap(markerPoses, chosenCameraPose, isRearPose, /*drawCamera=*/true,
							mapSize, scale, mapOrigin, arrowLength, outRear.Heading);


						//double trans_x = chosenCameraPose.x + 2 * std::cos(outRear.Heading * CV_PI/180);
						//double trans_z = chosenCameraPose.z + 2 * std::sin(outRear.Heading * CV_PI/180);
						//std::cout << "trans_x: " << trans_x << ", " << "trans_z" << trans_z << std::endl;

						long x_val = static_cast<long>(chosenCameraPose.x * 1000);
						long y_val = static_cast<long>(chosenCameraPose.z * 1000);
						long t_val = static_cast<long>(outRear.Heading * 10);

						posPublisher.PublishVisionPos(x_val, y_val, t_val);

						//std::this_thread::sleep_for(std::chrono::seconds(1));

						//std::pair<long, std::string> backendResponse = sendGlobalPoseToBackend(chosenCameraPose, resultForBackend, basicInfo, outRear.Heading);
						...
						chosenCameraPose = outFront.final_poses[0];
						...
						long x_val = static_cast<long>(chosenCameraPose.x * 1000);
						long y_val = static_cast<long>(chosenCameraPose.z * 1000);
						long t_val = static_cast<long>(outFront.Heading * 10);
						...
						//std::pair<long, std::string> backendResponse = sendGlobalPoseToBackend(chosenCameraPose, resultForBackend, basicInfo, outFront.Heading);
						...
						mapImage = create2DMap(markerPoses, chosenCameraPose, isRearPose, /*drawCamera=*/true,
							mapSize, scale, mapOrigin, arrowLength, outFront.Heading);
					}
				}
				else if (F_D_T && !R_D_T) {
					// 마커 유무: 전면O, 후면X -> 전면 OUTPUT 가장 가까운 것
					chosenCameraPose = outFront.final_poses[0];
					std::cout << "front global pos: " << chosenCameraPose.x << ", " << chosenCameraPose.y << ", " << chosenCameraPose.z << std::endl;
					isRearPose = false;
					...
					// 마커 유무: 전면X, 후면O -> 후면 OUTPUT 가장 가까운 것
					...
					//chosenCameraPose.x = trans_x;
					//chosenCameraPose.z = trans_z;
			...
			try {
				...
				while (running) {
					...
					cv::Mat baseMap = create2DMap(markerPoses, PoseData{}, false, false,
						mapSize, scale, mapOrigin, arrowLength,0);
